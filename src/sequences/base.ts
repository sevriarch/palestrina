...
export default abstract class Sequence<ET extends SeqMember<unknown>> extends Collection<ET> implements ISequence<ET> {
    ...
    replaceIfWindow(size: number, step: number, finder: ArrayFinderFn<ET>, mapper: (r: ET[], i: number) => SeqArgument): this {
        if (typeof finder !== 'function') {
            throw new Error(`${this.constructor.name}.mapWindowIf(): requires a finder function`);
        }

        if (typeof mapper !== 'function') {
            throw new Error(`${this.constructor.name}.mapWindowIf(): requires a mapper function`);
        }
    ...
    replaceIfReverseWindow(size: number, step: number, finder: ArrayFinderFn<ET>, mapper: (r: ET[], i: number) => SeqArgument): this {
        if (typeof finder !== 'function') {
            throw new Error(`${this.constructor.name}.mapWindowReverseIf(): requires a finder function`);
        }
