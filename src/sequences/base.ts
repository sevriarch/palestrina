...
export default abstract class Sequence<ET extends SeqMember<unknown>> extends Collection<ET> implements ISequence<ET> {
    ...
    /**
     * Repeatedly replace 'size' consecutive values with new ones generated by the mapper function
     * acting on these values. After each replacement, the next replacement begins 'step' indices
     * further along in the Sequence.
     *
     * Unlike the Chunks method in Sequences, this method may act on a Sequence member, then act on
     * the modified value of that Sequence member. For example, this would happen if the size was
     * larger than the step size.
     *
     * This method starts from index 0 (or the supplied offset) and the acted-on windows will move
     * rightwards along the contents of the Sequence.
     */
    mapWindows(size: number, step: number, mapper: (r: ET[], i: number) => SeqArgument): this {
        if (typeof mapper !== 'function') {
            throw new Error(`${this.constructor.name}.mapWindows(): requires a mapper function`);
        }

        return this.replaceIfWindow(size, step, () => true, mapper);
    }

    /**
     * Repeatedly replace 'size' consecutive values with new ones generated by the mapper function
     * acting on these values, but only if these values pass the supplied filter function with the 
     * arguments ([ value1, value2, ... ], index). After each replacement, the next replacement
     * begins 'step' indices further along in the Sequence.
     *
     * Unlike the Chunks method in Sequences, this method may act on a Sequence member, then act on
     * the modified value of that Sequence member. For example, this would happen if the size was
     * larger than the step size.
     ...
     * )
     */
    replaceIfWindow(size: number, step: number, filter: ArrayFinderFn<ET>, mapper: (r: ET[], i: number) => SeqArgument): this {
        if (typeof filter !== 'function') {
            throw new Error(`${this.constructor.name}.mapWindowsIf(): requires a filter function`);
        }

        if (typeof mapper !== 'function') {
            throw new Error(`${this.constructor.name}.mapWindowsIf(): requires a mapper function`);
        }

        const vals = this.val();

        for (let i = this.length - size; i >= 0; i -= step) {
            const end = vals.length - i;
            ...
            if (filter(slice, start)) {
                const mapped = mapper(slice, start);

                vals.splice(start, size, ...this.replacerValue(mapped));
            }
        }
     ...
     * )
     */
    replaceIfReverseWindow(size: number, step: number, filter: ArrayFinderFn<ET>, mapper: (r: ET[], i: number) => SeqArgument): this {
        if (typeof filter !== 'function') {
            throw new Error(`${this.constructor.name}.mapWindowReverseIf(): requires a filter function`);
        }

        if (typeof mapper !== 'function') {
            throw new Error(`${this.constructor.name}.mapWindowReverseIf(): requires a mapper function`);
        }
