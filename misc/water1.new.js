...
const V_FF  = 90
const V_FFF = 100
const V_RANGE = V_FFF - V_PP

const DEBUG = process.env.WATER_DEBUG && process.env.WATER_DEBUG !== '0'

const base = noteseq(imports.rasmussen(LEN * 3))

const m1 = base.keepNth(2)
               .mod(33)
               .dedupe()
               .invert(8)
               .replaceIndices(0, null)
               .map((v, i) => (i > 120 && ((v.val() * 110) < (-760 - i))) ? v.transpose(8) : v)
               .scale('octatonic21', 0)
const m2 = base.dropNth(2)
               .mod(33)
               .dedupe()
               .transpose(-16)
               .retrograde()
...
function dynamics1(len) {
    function cresc(val) {
        return V_PP + Math.floor(val * V_RANGE / 2)
    }

    function dim(val) {
        return V_FFF - Math.floor(val * V_RANGE / 2)
    }

    function vol(ix) {
        let loc = 8 * ix / len

        if (loc < 0.5) { return cresc(loc * 2) }
        if (loc < 1) { return dim(loc * 2) }

        loc = loc % 8

        if (loc < 2) { return V_PP }
        if (loc < 4) { return cresc(loc - 2) }
        if (loc < 6) { return V_FFF }
        return dim(loc - 6)
    }

    return noteseq(imports.increment(len).map(vol))
}
